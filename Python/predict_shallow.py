...
def predict_shallow(sensor_data: str) -> str:
    ...
    file_name_with_ext = os.path.basename(sensor_data)
    # Split the file name by "_" and get the first part
    ...
    most_common_hgbc = c_hgbc.most_common()[0][0] #[:3]
    most_common_rfc = c_rfc.most_common()[0][0] #[:3]

    #print(f"'{most_common_hgbc}' and its first eight characters: '{most_common_hgbc[:8]}'")
    if most_common_hgbc[:8] == "SITtoLAY":
        most_common_hgbc = "LAY"
    else:
        most_common_hgbc = most_common_hgbc[:3]


    if most_common_rfc[:8] == "SITtoLAY":
        most_common_rfc = "LAY"
    else:
        most_common_rfc = most_common_rfc[:3]

    results = [most_common_hgbc, most_common_rfc]
